...
<script type="text/javascript" language="javascript">
	...
	function oK() {
		...
		window.location.href = "Admin_StudentManageQuery?currentPage="
...
<body>
	...
	<section id="main" class="column">
		...
		<article class="module width_full">
			...
			<form class="quick_search" action="Admin_StudentManageQuery"
				method="get">
							...
							<tr>
								<c:forEach var="student" items="${studentList}">
									<tr>
										...
										<td><c:out value="${student.grade}"></c:out>
										</td>
										<td><c:out value="${student.school}"></c:out>
										</td>
										<td><c:out value="${student.department}"></c:out>
										</td>
										<td><c:out value="${student.major}"></c:out>
										</td>
										<td><a href="Admin_ModifyStuInfoHref?stuId=${student.id}">
												<input type="image" src="images/icn_edit.png" title="修改">
										</a> <a href="Admin_DeleteStudent?stuId=${student.id}" onClick="delcfm()"> <input
												type="image" src="images/icn_trash.png" title="删除?">
										</a>
										</td>
									</tr>
								</c:forEach>
						...
						<thead>
							...
							<tr valign="middle">
								...
								<td colspan="5" class="bg_tr" align="right"><a
									href="Admin_StudentManageQuery?currentPage=1&keyword=<%=keyword%>">首页&nbsp;</a>
									<%
										if (currentPage > 1) {
									%> <a
									href="Admin_StudentManageQuery?currentPage=<%=currentPage - 1%>&keyword=<%=keyword%>">上一页&nbsp;</a>
									<%
										} else {
									%>上一页&nbsp;<%
										}
 ...
 %> <a
									href="Admin_StudentManageQuery?currentPage=<%=currentPage + 1%>&keyword=<%=keyword%>">下一页&nbsp;</a>
									<%
										} else {
									%>下一页&nbsp;<%
										}
									%> <a
									href="Admin_StudentManageQuery?currentPage=<%=allpage%>&keyword=<%=keyword%>">尾页&nbsp;</a>
