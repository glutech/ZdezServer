...
<script type="text/javascript" language="javascript">
	...
	function oK() {
		...
		window.location.href = "Admin_SchoolAdminManage?currentPage="
				+ currentPage;
	}
</script>
<script type="text/javascript" language="javascript">
...
<body>
						...
						<tbody>
							<tr>
								<c:forEach var="schoolAdmin" items="${schoolAdminList}">
									<tr>
										<td><c:out value="${schoolAdmin.name}" />
										</td>
										<td><c:out value="${schoolAdmin.auth}"></c:out>
										</td>
										<td><c:out value="${schoolAdmin.school}"></c:out>
										</td>
										<td><c:out value="${schoolAdmin.department}"></c:out>
										</td>
										<td><c:out value="${schoolAdmin.major}"></c:out>
										</td>
										<td><%--<a
											...
											onClick="delcfm()"> <input type="image"
												src="images/icn_trash.png" title="删除?"> </a>
										</td>
									</tr>
								</c:forEach>
							</tr>
						...
						<thead>
							...
							<tr valign="middle">
								...
								<td colspan="5" class="bg_tr" align="right"><a
									href="Admin_SchoolAdminManage?currentPage=1">首页&nbsp;</a> <%
 	if (currentPage > 1) {
 %> <a href="Admin_SchoolAdminManage?currentPage=<%=currentPage - 1%>">上一页&nbsp;</a>
									<%
										} else {
									%>上一页&nbsp;<%
										}
 ...
 %> <a href="Admin_SchoolAdminManage?currentPage=<%=currentPage + 1%>">下一页&nbsp;</a>
									<%
										} else {
									%>下一页&nbsp;<%
										}
									%> <a href="Admin_SchoolAdminManage?currentPage=<%=allpage%>">尾页&nbsp;</a>
