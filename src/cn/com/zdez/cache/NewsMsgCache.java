...
public class NewsMsgCache {
		...
		try {
			...
			for (int i = 0, count = list.size(); i < count; i++) {
				...
				if (list.get(i).getIsTop() == 1) {
					Set<String> newsIdListStr = jedis
							.smembers("newsMsg:idList");
				...
				for (int n = 0, count1 = list.get(i).getDestSchools().size(); n < count1; n++) {
					jedis.sadd("destSchool:" + list.get(i).getId(), list.get(i)
							.getDestSchools().get(n));
				}
			}
	...
	public NewsVo getFromCache(int newsId) {
		...
		try {
			newsVo.setId(newsId);
			String title = jedis.hget("newsMsg:" + Integer.toString(newsId),
					"title");
			String content = jedis.hget("newsMsg:" + Integer.toString(newsId),
					"content");
			String cover = jedis.hget("newsMsg:" + Integer.toString(newsId),
					"cover");
			String isTop = jedis.hget("newsMsg:" + Integer.toString(newsId),
					"isTop");

			String date = jedis.hget("newsMsg:" + Integer.toString(newsId),
					"date");

			Set<String> schoolSet = jedis.smembers("destSchool:"
					+ Integer.toString(newsId));
	...
	public List<NewsVo> getNewsMsgFromCache(List<Integer> idList) {
		...
		try {
			...
			if (idListGetMsgFromDB.size() > 0) {
				...
				newsMsgToCacheList = new NewsService()
						.getNewsByIdList(idListGetMsgFromDB);
	...
	public void cacheNewsMsg_ReceivedStu(int stuId, List<Integer> list) {
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				if (!jedis.sismember(key, Integer.toString(list.get(i)))) {
					jedis.sadd(key, Integer.toString(list.get(i)));
					jedis.hincrBy("newsMsg:receivedNum",
							Integer.toString(list.get(i)), 1);
				}
			}
