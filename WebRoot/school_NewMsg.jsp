...
<%!String genSelectForCC(List lis, String formFieldName, String id) {
		StringBuffer buf = new StringBuffer();
		buf.append("<select id='" + id + "' name='" + formFieldName
				+ "' multiple='multiple' style='width: 200px'>");
...
<%
	...
	List<Student> stuAffairsTeacherList = null;
	List<Student> empTeacherList = null;
	List<Student> yccTeacherList = null;
	List<Student> sdTeacherList = null;
	...
	stuAffairsTeacherList = (List<Student>)session.getAttribute("stuAffairsTeacherList");
	empTeacherList = (List<Student>)session.getAttribute("empTeacherList");
	yccTeacherList = (List<Student>)session.getAttribute("yccTeacherList");
	sdTeacherList = (List<Student>)session.getAttribute("sdTeacherList");
...
<script>
	$(function() {
		...
		$("#stuAffairs").multiselect({
			noneSelectedText: '学生处'
		});

		$("#employment").multiselect({
			noneSelectedText: '就业处'
		});
		...
		$("#securityDpt").multiselect({
			noneSelectedText: '保卫处'
		});
...
<script type="text/javascript">
	...
	$(window)
			.load(
					function() {
						var $body = $('body'), $header = $('#header'), $secondary_bar = $('#secondary_bar'), $sidebar = $('#sidebar'), $main = $('#main');
						...
						var resizeFunc = function() {
							...
							var contentHeight = function() {
								var bodyHeight = $body.height()
										- $header.height()
										- $secondary_bar.height() + 4 // 4px是#contentContainer上浮遮罩阴影的大小
								, sidebarHeight = $sidebar.outerHeight(true), mainHeight = $main
										.outerHeight(true);
								return maxNumFunc(bodyHeight, sidebarHeight,
										mainHeight);
							}();
	...
	$(function() {
		var $checkbox = $('#teachers :checkbox'), $p = $checkbox.closest(
				'fieldset').find('>p'), onCheckboxValueChanged = function(v) {
			v ? $p.show() : $p.hide();
		};
		$checkbox.click(function() {
			onCheckboxValueChanged($checkbox.prop('checked'));
		});
		$checkbox.closest('fieldset').find('label').click(
				function() {
					onCheckboxValueChanged(!$checkbox.prop('checked'));
					$checkbox.prop('checked') ? $checkbox.removeAttr('checked')
...
<body link="#FF0000" vlink="#0000FF" alink="#ff0000"
	...
	<div class="breadcrumbs_container">
		<article class="breadcrumbs">
			<a href="school.jsp">信息管理</a>
			<div class="breadcrumb_divider"></div>
			<a class="current">新建信息</a>
		</article>
	</div>
							...
							<td>
						      ...
						      <td></td>
									...
									<tr valign="top">
										<td colspan="2">
											<fieldset>
												<label style="font-size:12px">通知抄送</label>
												<%=genSelectForCC(stuAffairsTeacherList, "xueshengchu[]", "stuAffairs")%>
												<%=genSelectForCC(empTeacherList, "jiuyechu[]", "employment")%>
												<%=genSelectForCC(yccTeacherList, "tuanwei[]", "ycc")%>
												<%=genSelectForCC(sdTeacherList, "baoweichu[]", "securityDpt")%>
											</fieldset>
										</td>
									</tr>
									<tr valign="top">
										<td colspan="2">
											<fieldset>
												<label style="font-size:12px">信息标题</label> <input
													type="text" name="schoolmessagetitle"
	...
	<script type="text/javascript">
		...
		function validate_form() {
			var $schoolmessagetitle = $('#schoolmessagetitle'), $schoolSys = $('#schoolSys'), $grade = $('#grade'), $depart = $('#depart'), $major = $('#major'), $teacherSwith = $('#teachers input[type=checkbox]'), $teacher = $('#teacher');
			...
			if ($teacherSwith.prop('checked')
					&& $teacher.get(0).selectedIndex == -1) {
